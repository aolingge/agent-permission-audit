...
const checks = [
  {
    "id": "filesystem",
    "pattern": "file|filesystem|read|write|文件|读|写",
    "message": "Mentions file access."
  },
  {
    "id": "shell",
    "pattern": "shell|command|exec|terminal|命令|终端",
    "message": "Mentions shell command rules."
  },
  {
    "id": "network",
    "pattern": "network|browser|http|api|网络|浏览器",
    "message": "Mentions network or browser access."
  },
  {
    "id": "secrets",
    "pattern": "secret|token|cookie|credential|密钥|令牌|凭据",
    "message": "Mentions secret boundary."
  }
]
...
export function formatText(report, title = "Agent Permission Audit") {
...
export function formatMarkdown(report, title = "Agent Permission Audit") {
...
export function formatSarif(report, toolName = "agent-permission-audit") {
  return {
    version: '2.1.0',
